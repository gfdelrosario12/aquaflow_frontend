...
The system SHALL provide a shared REST client configured with an environment-specific base URL, JSON request/response handling, authentication header injection, request timeouts, response decoding, and safe request diagnostics.

#### Scenario: API service sends a configured request
- **WHEN** a repository invokes an API service
- **THEN** the client sends the request to the configured base URL with JSON headers, the current access token when available, and the configured timeout policy.
...
The system SHALL map timeout, connectivity, authentication, validation, server, and decoding failures into typed API/repository errors and SHALL apply bounded retries only to eligible transient idempotent requests.
