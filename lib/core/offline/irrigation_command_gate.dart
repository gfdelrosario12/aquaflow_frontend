...
class IrrigationCommandDecision {
  ...
  const IrrigationCommandDecision({required this.allowed, required this.message});
}
...
class IrrigationCommandGate {
  ...
  IrrigationCommandDecision check({
    ...
    if (!connectivity.canReachBackend || !authenticated || !controllerConfirmed) {
      return const IrrigationCommandDecision(
        allowed: false,
        message: 'Live backend and controller confirmation are required before irrigation commands.',
      );
    }
    if (confirmation != CacheConfirmation.live) {
      return const IrrigationCommandDecision(
        allowed: false,
        message: 'Cached or stale controller state cannot authorize irrigation commands.',
      );
    }
    return const IrrigationCommandDecision(
      allowed: true,
